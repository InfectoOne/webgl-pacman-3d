...
// Gourard shading is per-vertex shading. It doesn't look as good as Phong shading, but is computationally more eficcient
const GOURARD_SHADER = {
  ...
  vertexShaderCode: `
    ...
    void main() {
      ...
      // ==================    compute the ambient shading according to the Phong model:  ==================
      ...
      vec3 viewerDirection = (uModelViewMatrix * aVertexPosition).xyz;    // extract x,y and z to get a vec3 instead of a vec4
      vec3 nViewerDirection = normalize(-viewerDirection);
      vec3 nDistFromSource = normalize(lightPosition.xyz - viewerDirection);
      vec3 nNormal = normalize((uModelViewMatrix * aVertexNormal).xyz);
      vec4 diffuse;
      ...
      vec3 nReflectedRayDirection = normalize( uModelViewMatrix * aVertexNormal).xyz;
      vec4 specular;
      if(uIsSpecularEnabled){
        specular = pow(max(dot(nReflectedRayDirection, nTotalDist), 0.0), uShininess) * uLightSpecularColor * uMaterialSpecularColor;
...
// note that we only declare colors for vertices. The colors for the pixels in between are automatically interpolated
  fragmentShaderCode : `
    precision mediump float;
    varying lowp vec4 vColor; // the variable from the vertex shader above

    //  this functions gets called every time a pixel needs to be filled with color
    void main() {
      gl_FragColor = vColor;  // we could also hard-code a value here, then our fragment shader would ALWAYS use that color
    }
  `
}